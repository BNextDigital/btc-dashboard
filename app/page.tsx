...
const MetricTooltip = ({ metricId }: { metricId: string }) => {
  ...
  return (
    ...
    >
      <button className="flex items-center justify-center w-[13px] h-[13px] rounded-full border border-[#22231F] text-[#55534B] hover:text-[#B8B5AA] hover:border-[#55534B] transition-colors leading-none"
        style={{ fontSize: "8px" }}>
        ?
      </button>
      ...
      {visible && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 pointer-events-none"
          ...
          <div className="bg-[#0E0E10] border border-[#22231F] p-3 shadow-2xl">
            ...
            <p className="font-sans-body text-paper-2 leading-relaxed" style={{ fontSize: "11px" }}>
              {content.body}
            </p>
          </div>
          {/* Arrow */}
        ...
        <div className="grid grid-cols-2 gap-3 mt-1">
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              <div className="w-[7px] h-[7px] rounded-sm flex-shrink-0" style={{ backgroundColor: "#26A17B" }} />
              <span className="caps-sm text-faint">USDT · {data.usdt_share}%</span>
            </div>
            <div className="font-mono-data text-paper text-[13px]">{data.usdt}</div>
            <div className={`font-mono-data text-[11px] mt-0.5 ${data.usdt_7d.startsWith("+") ? "text-neutral-sage" : data.usdt_7d.startsWith("-") ? "text-alert-extreme" : "text-muted"}`}>
              {data.usdt_7d} 7d
            </div>
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              <div className="w-[7px] h-[7px] rounded-sm flex-shrink-0" style={{ backgroundColor: "#2775CA" }} />
              <span className="caps-sm text-faint">USDC · {data.usdc_share}%</span>
            </div>
            <div className="font-mono-data text-paper text-[13px]">{data.usdc}</div>
            <div className={`font-mono-data text-[11px] mt-0.5 ${data.usdc_7d.startsWith("+") ? "text-neutral-sage" : data.usdc_7d.startsWith("-") ? "text-alert-extreme" : "text-muted"}`}>
              {data.usdc_7d} 7d
            </div>
          </div>
        </div>
    ...
    <div className="bg-surface border hairline p-4 flex flex-col gap-3 hover:bg-surface-2 transition-colors duration-300">
      ...
      <div className="hairline-t pt-3 flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-3 mt-1">
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              <div className="w-[7px] h-[7px] rounded-sm flex-shrink-0" style={{ backgroundColor: "#D9A84D" }} />
              <span className="caps-sm text-faint">BTC · {data.btc_share}%</span>
            </div>
            <div className="font-mono-data text-paper text-[13px]">{data.btc_cap}</div>
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              <div className="w-[7px] h-[7px] rounded-sm flex-shrink-0" style={{ backgroundColor: "#55534B" }} />
              <span className="caps-sm text-faint">Alts · {data.alt_share}%</span>
            </div>
          ...
          <li key={i} className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="caps-sm text-faint">{ev.tag}</span><span className="text-faint">·</span>
                <span className="caps-sm text-faint">{ev.source}</span><span className="text-faint">·</span>
  ...
  return (
    <div className="bg-surface border hairline p-5 flex flex-col h-full">
        ...
        {chain.map((c, i) => (
          <div key={c.label} className="flex items-start gap-4 py-2.5" style={{ borderTop: i > 0 ? "1px solid #1A1A1C" : "none" }}>
            ...
            <div className="flex-1">
              ...
              <div className={`font-sans-body text-[12px] italic ${weightColor[c.weight] ?? "text-paper-2"} mt-0.5`}>{c.state}</div>
            </div>
      ...
      <div className="hairline-t pt-4 mt-5">
        ...
        <p className="font-display-italic text-paper text-[16px] leading-snug">{contradiction}</p>
      </div>
  ...
  const handleCommit = async () => {
    ...
    try {
      const res = await fetch(`${API}/judgment`, {
        ...
        body: JSON.stringify({ read: state.read, supports: state.supports, contradicts: state.contradicts, invalidates: state.invalidates, plan: state.plan, risk: state.risk }),
      });
    ...
    <div className="bg-surface border hairline p-5 flex flex-col h-full">
      ...
      <div className="flex flex-col gap-3 flex-1">
        {fields.map((f) => (
          <div key={f.key}>
            ...
            <textarea rows={f.rows} value={state[f.key] || ""}
              onChange={(e) => { setState((s) => ({ ...s, [f.key]: e.target.value })); setCommitted(null); setCommitError(null); }}
              className="w-full bg-surface-inset border hairline px-2.5 py-2 text-paper text-[12px] font-sans-body resize-none" placeholder="..." />
          </div>
        ))}
        <div>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {risks.map((r) => (
              <button key={r} onClick={() => setState((s) => ({ ...s, risk: r }))}
      ...
      <div className="mt-4 hairline-t pt-3 flex items-center justify-between">
        ...
        <button onClick={handleCommit} disabled={committing}
          className={`caps-sm px-3 py-1.5 border transition-colors ${committing ? "border-faint text-faint cursor-not-allowed" : "border-amber-sand text-amber-sand hover:bg-amber-sand-10"}`}>
  ...
  return (
    <div className="bg-surface border hairline">
      <div className="flex items-center justify-between px-5 py-4 hairline-b">
        <div><div className="caps-sm text-faint">Manual override</div><h2 className="font-display text-paper text-[22px] leading-tight mt-0.5">Screenshot → data</h2></div>
        <div className="flex items-center gap-2"><Circle size={7} fill="#D9A84D" stroke="none" /><span className="caps-sm text-amber-sand">Paste Claude output</span></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 divide-x hairline">
        <div className="p-5">
          ...
          <textarea rows={12} value={input} onChange={(e) => { setInput(e.target.value); setStatus("idle"); setMessage(null); }}
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <button onClick={handleSubmit} disabled={saving || !input.trim()} className={`caps-sm px-4 py-2 border transition-colors ${saving || !input.trim() ? "border-faint text-faint cursor-not-allowed" : "border-amber-sand text-amber-sand hover:bg-amber-sand-10"}`}>{saving ? "Updating…" : "Apply override"}</button>
          </div>
        </div>
...
const ActiveOverrides = ({ onClear }: { onClear: (metric: string) => void }) => {
  ...
  if (entries.length === 0) return (
    <div className="flex flex-col gap-3">
      ...
      <p className="font-sans-body text-muted text-[11px] leading-relaxed">Paste Claude's JSON output on the left and click Apply override.</p>
    </div>
  );
  return (
    <div className="flex flex-col gap-3">
        ...
        return (
          <div key={key} className="bg-surface-inset border hairline p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2"><Circle size={5} fill="#D9A84D" stroke="none" className="pulse-dot" /><span className="font-sans-body text-paper text-[12px] font-medium">{val.name}</span></div>
...
const TradeExecutionPanel = ({ executions, onAdd }: { executions: any[]; onAdd: () => void }) => {
  ...
  const stateColors: Record<string, string> = { Green: "text-neutral-sage border-sage bg-sage-10", Yellow: "text-alert-notable border-notable bg-notable-10", Red: "text-alert-extreme border-extreme bg-extreme-10" };

  return (
    <div className="bg-surface border hairline">
      <div className="flex items-center justify-between px-5 py-4 hairline-b">
        <div><div className="caps-sm text-faint">VI</div><h2 className="font-display text-paper text-[22px] leading-tight mt-0.5">Trade execution</h2></div>
        <div className="flex items-center gap-4">
          <span className="caps-sm text-faint">{executions.length} entries</span>
          <button onClick={() => { setShowForm(!showForm); setSaveError(null); }} className={`caps-sm px-3 py-1.5 border transition-colors ${showForm ? "border-amber-sand bg-amber-sand-10 text-amber-sand" : "hairline text-muted hover:text-paper hover:border-amber-sand"}`}>{showForm ? "Cancel" : "Add trade"}</button>
        </div>
      </div>
      {showForm && (
        <div className="px-5 py-5 hairline-b bg-surface-2">
          ...
          {volume > 0 && (
            <div className="hairline-t pt-4 mb-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-surface-inset border hairline p-3"><div className="caps-sm text-faint mb-1">0.5x — Slowdown</div><div className="font-mono-data text-paper-2 text-[14px]">{fmt(vol05x)} BTC</div></div>
                <div className="bg-surface-inset border hairline p-3"><div className="caps-sm text-faint mb-1">1.5x — Interest</div><div className="font-mono-data text-amber-sand text-[14px]">{fmt(vol15x)} BTC</div></div>
                <div className="bg-surface-inset border hairline p-3"><div className="caps-sm text-faint mb-1">2.0x — Significant</div><div className="font-mono-data text-alert-notable text-[14px]">{fmt(vol20x)} BTC</div></div>
              </div>
            </div>
          )}
          <div className="flex justify-end gap-3">
            ...
            <button onClick={handleSave} disabled={saving} className={`caps-sm px-3 py-1.5 border transition-colors ${saving ? "border-faint text-faint cursor-not-allowed" : "border-amber-sand text-amber-sand hover:bg-amber-sand-10"}`}>{saving ? "Saving…" : "Save execution"}</button>
          </div>
        </div>
      )}
      <div className="grid grid-cols-12 caps-sm text-faint px-5 py-2.5 hairline-b bg-surface-inset">
        <div className="col-span-1">Date</div><div className="col-span-1">State</div><div className="col-span-2">Planned</div><div className="col-span-2">Actual</div><div className="col-span-1">Slip</div><div className="col-span-1">Size</div><div className="col-span-1">Stop%</div><div className="col-span-2">Stop $</div><div className="col-span-1">Vol BTC</div>
      </div>
      {executions.length === 0 ? (
        <div className="px-5 py-8 text-center"><span className="caps-sm text-faint">No execution entries yet — add your first trade above</span></div>
      ) : (
        executions.map((e, i) => {
          const sc = e.market_state === "Green" ? "text-neutral-sage" : e.market_state === "Yellow" ? "text-alert-notable" : e.market_state === "Red" ? "text-alert-extreme" : "text-muted";
          return (
            <div key={i} className={`grid grid-cols-12 px-5 py-3 text-[12px] font-sans-body items-center ${i < executions.length - 1 ? "hairline-b" : ""} hover:bg-surface-2 transition-colors`}>
              ...
              <div className={`col-span-1 caps-sm ${sc}`}>{e.market_state}</div>
      ...
      <div className="px-5 py-4 hairline-t bg-surface-inset flex items-center justify-between">
        <div className="flex items-center gap-2 text-faint"><Activity size={12} /><span className="caps-sm">Quantitative execution log · feeds SEM analytics system</span></div>
...
const TradeLogReview = ({ logs, onAdd }: { logs: TradeLog[]; onAdd: () => void }) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true); setSaveError(null);
    try {
      const res = await fetch(`${API}/trade-log`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      if (!res.ok) throw new Error(`Server returned ${res.status}`);
      setShowForm(false); setForm({ structure: "", capital: "", read: "", contradiction: "", plan: "", risk: "" }); onAdd();
    } catch (e) { setSaveError(e instanceof Error ? e.message : "Save failed"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="bg-surface border hairline">
      <div className="flex items-center justify-between px-5 py-4 hairline-b">
        <div><div className="caps-sm text-faint">IV</div><h2 className="font-display text-paper text-[22px] leading-tight mt-0.5">Review & notes</h2></div>
        <div className="flex items-center gap-4">
          <span className="caps-sm text-faint">{logs.length} entries</span>
          <button onClick={() => setShowForm(!showForm)} className={`caps-sm px-3 py-1.5 border transition-colors ${showForm ? "border-amber-sand bg-amber-sand-10 text-amber-sand" : "hairline text-muted hover:text-paper hover:border-amber-sand"}`}>{showForm ? "Cancel" : "New entry"}</button>
        </div>
      </div>
      {showForm && (
        <div className="px-5 py-4 hairline-b bg-surface-2">
          <div className="mb-4"><div className="caps-sm text-amber-sand mb-1">Log a trade decision</div><p className="font-sans-body text-muted text-[11px]">Record what the market looked like, what you decided, and why.</p></div>
          {saveError && <div className="bg-extreme-10 border border-extreme px-3 py-2 mb-3"><span className="caps-sm text-alert-extreme">{saveError}</span></div>}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            {formFields.map((f) => (<div key={f.key}><label className="caps-sm text-faint block mb-1.5">{f.label}</label><textarea rows={2} value={form[f.key]} onChange={(e) => setForm((s) => ({ ...s, [f.key]: e.target.value }))} placeholder={f.placeholder} className="w-full bg-surface-inset border hairline px-2.5 py-2 text-paper text-[12px] font-sans-body resize-none" /></div>))}
          </div>
          <div className="flex justify-end gap-3">
            ...
            <button onClick={handleSave} disabled={saving} className={`caps-sm px-3 py-1.5 border transition-colors ${saving ? "border-faint text-faint cursor-not-allowed" : "border-amber-sand text-amber-sand hover:bg-amber-sand-10"}`}>{saving ? "Saving…" : "Log This Trade"}</button>
          </div>
        </div>
      )}
      <div className="grid grid-cols-12 caps-sm text-faint px-5 py-2.5 hairline-b bg-surface-inset">
        <div className="col-span-1">Date</div><div className="col-span-1">Price</div><div className="col-span-2">Structure</div><div className="col-span-3">Read</div><div className="col-span-2">Plan</div><div className="col-span-2">Result</div><div className="col-span-1">Bias</div>
      </div>
      {logs.length === 0 ? (
        <div className="px-5 py-8 text-center"><span className="caps-sm text-faint">No entries yet — add your first trade log above</span></div>
      ) : (
        logs.map((log, i) => (
      ...
      <div className="px-5 py-4 hairline-t flex items-center justify-between bg-surface-inset">
        <div className="flex items-center gap-2 text-faint"><FileText size={12} /><span className="caps-sm">Post-trade SEM review · run weekly with Claude</span></div>
        <button className="caps-sm text-amber-sand hover:underline flex items-center gap-1">Run review <ChevronRight size={11} /></button>
      </div>
    </div>
  );
};
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        if (Array.isArray(tradeLogData) && tradeLogData.length > 0) setLogs(tradeLogData);

        const execRes  = await fetch(`${API}/trade-execution`);
        const execData = await execRes.json();
        if (Array.isArray(execData)) setExecutions(execData);

        // Capture raw stablecoin data before transforming
        if (data["stablecoin_supply"]) {
          setStablecoinData(data["stablecoin_supply"] as StablecoinData);
        }
        ...
        <main className="max-w-[1440px] mx-auto px-8 py-8 space-y-10">
          ...
          <div className="flex flex-wrap items-center gap-6 bg-surface border hairline px-5 py-4">
            ...
            <div className="ml-auto flex items-center gap-5">
              ...
              <div className="flex items-center gap-2 pl-5 border-l hairline"><Clock size={11} className="text-faint" /><span className="caps-sm text-faint">{now.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</span></div>
            </div>
          </div>
